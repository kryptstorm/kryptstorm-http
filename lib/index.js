...
var _preparePayload = function _preparePayload(req, rest) {
  ...
  var _query$_limit = query._limit,
      _limit = _query$_limit === undefined ? 20 : _query$_limit,
      _query$_page = query._page,
      _page = _query$_page === undefined ? 1 : _query$_page,
      _sort = query._sort,
  ...
  switch (method) {
    case "POST":
      _payload.body = _lodash2.default.isObject(body) ? body : {};
      break;
    case "GET":
      _payload.query = _preapreQuery(query, ["_limit", "_page", "_sort", "_accessToken", "_refreshToken"]);
      _lodash2.default.assign(_payload.query, _prepareSort(_sort), _preparePagination(_limit, _page));
...
var _prepareSort = function _prepareSort(sort) {
  ...
  return {
    sort$: _lodash2.default.reduce(sort.split(","), function (_sort, field) {
...
var _preparePagination = function _preparePagination(limit, page) {
  var p = { limit$: 20, skip$: 0 };

  if (_lodash2.default.isNumber(Number(limit)) && limit > 0) {
    p.limit$ = limit;
  }
  if (_lodash2.default.isNumber(Number(page)) && page > 0) {
